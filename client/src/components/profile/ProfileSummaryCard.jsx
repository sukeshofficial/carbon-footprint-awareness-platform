...
import {
  USER_TYPES_LIST,
  TONE_PREFERENCES_LIST,
  HOUSEHOLD_TYPES_LIST
} from './constants';
...
const ProfileSummaryCard = ({ profile }) => {
  ...
  const userType =
    USER_TYPES_LIST.find((t) => t.id === profile.userType)?.title ||
    profile.userType;

  const tonePref =
    TONE_PREFERENCES_LIST.find((t) => t.id === profile.tonePreference)?.title ||
    profile.tonePreference;

  const householdType =
    HOUSEHOLD_TYPES_LIST.find((h) => h.id === profile.householdType)?.title ||
      ...
      <CardHeader>
        <CardTitle className="flex justify-between items-center">
          ...
          <Badge variant="outline">
            Age group: {profile.ageGroup || 'Age Unknown'}
          </Badge>
        </CardTitle>
      </CardHeader>
...
ProfileSummaryCard.propTypes = {
  profile: PropTypes.shape({
    ...
    tonePreference: PropTypes.string,
    householdType: PropTypes.string,
  }),
};
