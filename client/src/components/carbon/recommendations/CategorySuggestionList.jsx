  ...
  return (
    <div className="space-y-12">
      {categories.map((cat) => {
        ...
        return (
          <div key={cat.id} className={cn("space-y-6", actions.length === 0 && "opacity-40")}>
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                <div className={cn("w-10 h-10 rounded-2xl flex items-center justify-center", cat.color)}>
                  <Icon size={20} />
                </div>
                <h3 className="text-xl font-black text-slate-800 dark:text-zinc-100 tracking-tight">{cat.label} Suggestions</h3>
              </div>
              <div className="text-[10px] font-black text-slate-400 uppercase tracking-widest">
                {actions.length} {actions.length === 1 ? 'Action' : 'Actions'} available
              </div>
            </div>
            ...
            <div className="relative">
              <div className="flex overflow-x-auto pb-6 -mx-4 px-4 gap-6 no-scrollbar snap-x">
                {actions.length > 0 ? (
                  actions.map((action) => (
                    <div key={action._id} className="min-w-[320px] md:min-w-[400px] snap-start">
                      <RecommendationCard
