...
export default function ScenarioInputPanel({ template, inputPayload, onUpdateInput }) {
  ...
  return (
    <div className="bg-white dark:bg-zinc-900 rounded-2xl border border-slate-100 dark:border-zinc-800 p-6">
      <div className="flex items-center gap-2 mb-5">
        ...
        <div>
          <h3 className="text-sm font-black text-slate-900 dark:text-zinc-100">Customize Scenario</h3>
          <p className="text-[10px] uppercase tracking-widest text-muted-foreground font-bold">Adjust parameters</p>
        </div>
      </div>
              ...
              {input.type === 'slider' && (
                <>
                  <input
                    ...
                    max={input.max}
                    value={value}
                    onChange={(e) => onUpdateInput(input.key, Number(e.target.value))}
