  ...
  if (isStreaming && !insights) {
    tipsContent = (
      <div className="space-y-4">
        ...
        {[1, 2].map((i) => (
          <div key={`stream-loading-${i}`} className="h-24 bg-white/50 border border-slate-100 rounded-3xl animate-pulse" />
        ))}
      </div>
    );
  } else if (insights?.tips?.length > 0) {
    tipsContent = (
    ...
    tipsContent = (
      <div className="space-y-4 animate-pulse">
        {[1, 2, 3].map((i) => (
          <div key={`tips-fallback-pulse-${i}`} className="h-24 bg-white border border-slate-100 rounded-3xl" />
        ))}
      </div>
    );
  }
