    ...
    return (
      <div className={cn(
        ...
        <div className="space-y-2 pt-2">
          {[1, 2, 3].map((i) => (
            <div key={i} className="h-3 w-3/4 bg-zinc-50 dark:bg-zinc-800/50 rounded-md" />
          ))}
        </div>
      </div>
    );
        ...
        {checklist.map((item, index) => (
          <div key={index} className="flex items-center gap-2.5 group/item">
            ...
            <span className={cn(
              ...
              item.completed ? "text-zinc-900 dark:text-zinc-100" : "text-zinc-400 dark:text-zinc-500"
            )}>
              {item.label}
            </span>
          </div>
      ...
      <div className="space-y-2 mb-4">
        <div className="flex items-end justify-between">
          <div className="space-y-0.5">
            <span className="block font-mono-tight text-[10px] font-bold text-zinc-400 dark:text-zinc-500 uppercase tracking-widest">{phase}</span>
            <span className="block text-sm font-bold text-zinc-900 dark:text-zinc-50">Completion</span>
          </div>
          <span className="text-2xl font-black text-green-600 tracking-tighter">{progressPercent}%</span>
        </div>

        <div
          className="h-2 w-full bg-zinc-50 dark:bg-zinc-800 rounded-full overflow-hidden border border-zinc-100 dark:border-zinc-800"
          role="progressbar"
        ...
        {nextMilestone && (
          <div className="flex items-center gap-2">
            ...
            <div className="flex flex-col">
              <span className="font-mono-tight text-[9px] font-bold text-zinc-400 dark:text-zinc-500 uppercase">Next Milestone</span>
              <span className="text-[11px] font-bold text-zinc-900 dark:text-zinc-100">{nextMilestone}</span>
            </div>
          </div>
        )}
