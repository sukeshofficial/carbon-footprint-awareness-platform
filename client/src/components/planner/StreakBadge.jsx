...
const StreakBadge = ({ currentStreak, longestStreak, className }) => {
  return (
      ...
      <div className="relative group">
        <div className={cn(
          ...
          <div className={cn(
            ...
            <Flame className={cn(
              ...
              currentStreak > 0 ? "text-white animate-pulse" : "text-gray-300"
            )} />
          </div>
        </div>
        ...
        {currentStreak > 0 && (
          <div className="absolute -top-1 -right-1 bg-white border border-orange-100 shadow-sm rounded-full w-6 h-6 flex items-center justify-center">
            <span className="text-[11px] font-bold text-orange-600">{currentStreak}</span>
          </div>
        )}
      </div>

      <div className="text-center">
        <p className="text-[10px] uppercase font-bold text-muted-foreground tracking-tighter">Current Streak</p>
        <p className="text-xs font-semibold">{currentStreak} {currentStreak === 1 ? 'day' : 'days'}</p>
      </div>

      {longestStreak > currentStreak && (
        <div className="mt-1 px-2 py-0.5 bg-orange-50 rounded-full border border-orange-100">
          <p className="text-[9px] font-medium text-orange-700">Best: {longestStreak}</p>
        </div>
      )}
    </div>
  );
