...
const GoalProgress = ({ goal, analytics = null, className = '' }) => {
  ...
  return (
    <div
      className={cn(
        'space-y-4 p-5 rounded-2xl bg-primary/5 border border-primary/10',
        className
      )}
    >
      ...
      <div className="space-y-2">
        ...
        <div className="flex justify-between items-center text-xs font-bold text-muted-foreground">
          <span>
            {current} {targetTypeLabels[goal.targetType]}
          </span>
        ...
        <div className="pt-2 flex items-center gap-4 border-t border-primary/10">
          <div>
            ...
            <p className="font-bold text-green-600">
              {analytics.estimatedCo2Saved.toFixed(1)} kg CO2e saved
            </p>
          </div>
...
GoalProgress.propTypes = {
  ...
  analytics: PropTypes.shape({
    goalAchievementPercent: PropTypes.number,
    estimatedCo2Saved: PropTypes.number,
